...
Geography::GeographicalCalculation::GeographicalCalculation()
{
}
...
///</summary>
double Geography::GeographicalCalculation::harversine(const Geography::LatLng& ll1, const Geography::LatLng& ll2)
{
...
/// 参照(http://www.movable-type.co.uk/scripts/latlong.html)
///</summary>
double Geography::GeographicalCalculation::harversine_distance(const Geography::LatLng& ll1, const Geography::LatLng& ll2)
{
...
///</summary>
double Geography::GeographicalCalculation::lambert_distance(const Geography::LatLng& ll1, const Geography::LatLng& ll2)
{
	//扁平率と極半径
...
/// 参照(http://www2.nc-toyama.ac.jp/~mkawai/lecture/sailing/geodetic/geosail.html)
///</summary>
double Geography::GeographicalCalculation::lambert_azimuth_angle(const Geography::LatLng& from, const Geography::LatLng& to)
{
...
std::vector<std::shared_ptr<Graph::Coordinate const>> convert_to_cartesian(const std::vector<std::shared_ptr<Geography::LatLng const>>& point_list)
{
	...
	for (; iter != point_list.end(); iter++) {
		double distance = Geography::distance(*reference_point, **iter);
		double azimuth_angle = Geography::azimuth_angle(*reference_point, **iter);
...
/// point_listが空の場合は空のvectorが返されます
///</summary>
const std::vector<std::shared_ptr<Geography::LatLng const>> Geography::GeographicalCalculation::convex_hull(const std::vector<std::shared_ptr<Geography::LatLng const>>& point_list)
{
...
///</summary>
double Geography::GeographicalCalculation::calc_convex_hull_size(const std::vector<std::shared_ptr<Geography::LatLng const>> &point_list)
{
	std::vector<std::shared_ptr<Graph::Coordinate const>> cartesian_points = convert_to_cartesian(point_list);
