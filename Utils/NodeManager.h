...
namespace Graph {
	...
	public:
		NodeManager();
		virtual ~NodeManager();
		std::shared_ptr<Graph::Node<NODE_DATA, EDGE_DATA>> get_node(node_id id);
		node_id create_node(NODE_DATA data);
		bool connect_to(node_id from, node_id to, EDGE_DATA data);
		bool connect_each_other(node_id id1, node_id id2, EDGE_DATA data1, EDGE_DATA data2);
		void disconnect_from(node_id target, node_id from);
		void disconnect_each_other(node_id id1, node_id id2);
		void sort_node_collection();
	};
}
