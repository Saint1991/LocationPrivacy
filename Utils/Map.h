...
namespace Geography 
{
	...
	protected:
		...
		virtual void create_routing_table();
	...
	public:
		Map(std::unique_ptr<Graph::RoutingMethod<NODE, PATH>> routing_method);
		virtual ~Map();
		...
		std::shared_ptr<const Geography::GeoNode<NODE, PATH>> get_static_node(const Graph::node_id& id) const;
		std::shared_ptr<Geography::GeoNode<NODE, PATH>> get_node(const Graph::node_id& id);
	};
}
