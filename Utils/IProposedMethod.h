...
namespace Framework
{
	...
	protected:
		...
		std::shared_ptr<Entity::EntityManager<POSITION_TYPE, TRAJECTORY_TYPE, DUMMY_TYPE, USER_TYPE>> entities;
		std::shared_ptr<Time::Timer> timer;

		//このfunctionに各回が終わった際の処理を記述する
		std::function<void(std::shared_ptr<Entity::EntityManager<POSITION_TYPE, TRAJECTORY_TYPE, DUMMY_TYPE, USER_TYPE>>, std::shared_ptr<REQUIREMENT_TYPE const>, std::shared_ptr<Time::Timer>)> execution_callback = nullptr;
	...
	public:
		...
		void set_execution_callback(const std::function<void(std::shared_ptr<Entity::EntityManager<POSITION_TYPE, TRAJECTORY_TYPE, DUMMY_TYPE, USER_TYPE>>, std::shared_ptr<REQUIREMENT_TYPE const>, std::shared_ptr<Time::Timer>)>& callback);
		void run();
	};
